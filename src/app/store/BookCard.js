...
const BookCard = ({ data }) => {
    ...
    return (
        <div className="rounded-t-2xl rounded-b-xl bg-white shadow-2xl md:w-80 w-72">
            <Image className="md:w-80 md:h-96 w-72 h-80 rounded-t-2xl" src={image} alt={title} width={200} height={50} />
            <div className="flex flex-col space-y-1 mt-1 p-2">
                ...
                <Link href={`/store/${_id}`}><PurpleButton className="w-full">See Details</PurpleButton></Link>
            </div>
        </div>
    );
};
